  ...
  return (
    <div className="relative">
      <input
        ...
        className="flex h-8 w-full sm:w-48 md:w-64 lg:w-[28rem] bg-gray-100 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
      />
      {filteredStocks.length > 0 && (
        <ul className="absolute z-10 w-full sm:w-48 md:w-64 lg:w-[32rem] bg-white border border-gray-300 rounded-md mt-2 max-h-60 overflow-y-auto shadow-lg scrollbar-hide">
          {filteredStocks.map((stock) => (
            ...
            >
              ...
              <div className="text-right flex items-center space-x-2">
                ...
                <div
                  className={`text-md ${stock.changepct >= 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"} rounded-md px-2 py-1`}
                >
                  {stock.changepct}%
                </div>
              </div>
